...
namespace Fusion.Addons.ConnectionManagerAddon
{
...
#endif
    {
        ...
        {
            _userName = AvatarSelection.instance.usname;
        ...
        public void OnPlayerJoinedSharedMode(NetworkRunner runner, PlayerRef player)
        {
            ...
            {
                string avatarName = AvatarSelection.instance.avatarPath;
                GameObject avatarPrefab = LoadAvatarPrefab(avatarName);
                // Spawn the user prefab for the local user
                NetworkObject networkPlayerObject = runner.Spawn(avatarPrefab, position: transform.position, rotation: transform.rotation, player, (runner, obj) =>
                {
                });
            }
        }
        private GameObject LoadAvatarPrefab(string avatarName)
        {
            if (!string.IsNullOrEmpty(avatarName))
            {
                return Resources.Load<GameObject>(System.IO.Path.Combine(avatarsFolder, avatarName));
            }
            return null;
        }
